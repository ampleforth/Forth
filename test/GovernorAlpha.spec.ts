...
describe('GovernorAlpha', () => {
  ...
  it('forth', async () => {
    const balance = await forth.balanceOf(wallet.address)
    const totalSupply = await forth.totalSupply()
  ...
  it('governor', async () => {
    ...
    const forthFromGovernor = await governorAlpha.forth()
    expect(forthFromGovernor).to.be.eq(forth.address)
  })
})
